...
void MainWindow::paintEvent(QPaintEvent *event ){
    ...
    if(color_num >= 10&&color_num < 20){
        brush.setColor(Qt::yellow);
    }
    if(color_num >= 20&&color_num < 30){
        brush.setColor(Qt::red);
    }
    color_num++;
    if(color_num>=30)color_num = 0;
    p.setPen(pen);
    p.setBrush(brush);
    ...
    QFont font("黑体",20,QFont::ExtraLight,false);
    pen.setColor(Qt::black);
    p.setPen(pen);
    p.setFont(font);
    QString qs = tr("%1").arg(game_time);
    QString sc = tr("%1").arg(score);

    p.drawText(1000,60,QString("Time：")+ qs) ;
    p.drawText(1000,100,QString("Score：") + sc);

    if(strike()){
        end_game = 1;
        QFont font("宋体",50,QFont::ExtraLight,1);
        p.setFont(font);
        p.drawText((this->width()-300)*2/5,(this->height()-30)/2,QString("GAME OVER!"));
        play_time = 0;
    }
...
void MainWindow::mousePressEvent(QMouseEvent*event){
   ...
   // std::cout<<"found!!!"<<" "<<temp.x()<<std::endl;
    ...
    for(int i = 0; i < barr.size(); i++){
        if(barr.at(i) == ref){
            if(barr.at(i).y() == 40||barr.at(i).y() == 820||barr.at(i).x() == 0||barr.at(i).x() == 780)return;
            barr.removeAt(i);
            QWidget:: repaint();
            return;
        }
...
void MainWindow::Read(){
    ...
    score = rootObj.take("score").toInt();
